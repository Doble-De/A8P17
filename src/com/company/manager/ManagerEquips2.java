...
public class ManagerEquips2 {
    ...
    public static int obtenirNumeroEquipsPerNom(String nom) throws IOException {
        int count = 0;
        FileChannel fc = (FileChannel.open(FileSystems.getDefault().getPath("equips.txt"), READ, WRITE, CREATE));
        long posFinal = fc.size();
        int recorrer = 0;
        ...
        while (recorrer < posFinal) {
            ...
            String nombre = new String(byteBufferNom.array(), Charset.forName("UTF-8"));
            int larg = nom.length();
            String nomb = nombre.substring(0, larg);ByteBuffer byteBufferNum = ByteBuffer.allocate(4);
